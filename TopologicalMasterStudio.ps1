...
$tabControl.Controls.Add($tabNephilimMesh)
...
function Read-MeshStateJson {
    $meshStateCandidates = @(
        ...
        "$HOME\Projects\compute-mesh\deno_app\mesh_state.json",
        "C:\sovereign_manifold\santos-sync\topological_system_optimizer\mesh_state.json",
        "C:\sovereign_manifold\santos-sync\topological_system_optimizer\deno_app\mesh_state.json",
        ".\mesh_state.json",
        ".\deno_app\mesh_state.json"
    )

    $foundFile = $null
    foreach ($cand in $meshStateCandidates) {
        if (Test-Path $cand) {
            $foundFile = $cand
            break
        }
    ...
    if ($foundFile) {
        try {
            $rawJson = Get-Content -Path $foundFile -Raw -Encoding utf8
            $txtMeshStateOut.Text = "[+] Found mesh_state.json at '$foundFile':`r`n`r`n" + $rawJson
        } catch {
            $txtMeshStateOut.Text = "[!] Error parsing ${foundFile}: $_"
        }
    } else {
        $defaultState = @{
            MeshVersion = "25.0.0"
            ...
            MajoranaParityLock = "Tr(U_res) = 1.000000"
            SheafCohomology = "H^1(U, F) = 0"
            LandauerFloor = "1.44 Joules"
            TensorVaultCapacity = "4,672 custom tensors (9.29 MB)"
            ActiveNodes = @("Master-Host (Local)", "Pixel-10-NPU (Socket 8088)")
            SocketPorts = @{ SignalingServer = 8080; WebGPU = 8090; LLMProxy = 8095; SwarmMesh = 8098; RESTGateway = 8099 }
            Note = "Active mesh_state.json will be written live once 'Start Deno Server Mesh' is executed."
        } | ConvertTo-Json -Depth 4

        $txtMeshStateOut.Text = "[!] Active mesh_state.json not yet written on disk.`r`n[+] Canonical Sheaf Mesh State Baseline:`r`n`r`n" + $defaultState
    }
}

function Read-MnemosyneDbInfo {
    $dbCandidates = @(
        "$HOME\Projects\compute-mesh\deno_app\mnemosyne.db",
        "$HOME\Projects\compute-mesh\mnemosyne.db",
        "C:\sovereign_manifold\santos-sync\topological_system_optimizer\deno_app\mnemosyne.db",
        "C:\sovereign_manifold\santos-sync\topological_system_optimizer\mnemosyne.db",
        ".\deno_app\mnemosyne.db",
        ".\mnemosyne.db"
    )

    $foundDb = $null
    foreach ($cand in $dbCandidates) {
        if (Test-Path $cand) {
            $foundDb = $cand
            break
        }
